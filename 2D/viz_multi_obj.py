...
Func = pickle.load( open( "SAVED_DATA/Opt_multi-obj_L3_1_10.pkl", "rb" ) )
...
phi,_ = Hicken_eval(pts,KDTree(Func.exact[0]),Func.exact[1],15,50)
...
ax1.plot([x[0],x[1]],[np.mean(y),np.mean(y)],color='cyan',linewidth=2)
ax1.set_xlabel('x')
ax1.set_ylabel('y')
...
for i,rng in enumerate(max_rngs):
    ind = xspan>rng[0]
    ind *= xspan<rng[1]
    vals = np.vstack((vals,np.max(phi_ex[ind])))
    ind = np.where(phi_ex==np.max(phi_ex[ind]))[0][0]
    if i==1:
        ind = np.where(phi_ex==np.max(phi_ex[ind]))[0][-1]
    locs = np.vstack((locs,xspan[ind]))  
...
plt.savefig('PDF_figures/multi_obj.pdf',bbox_inches='tight') 
# plt.show()